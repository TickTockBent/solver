...
Scored against the spec's criteria: **"one axis dominates," and it is decisively
depth.**
...
3. **No compounding.** Interaction ≈ 0, turning slightly negative at the
   ...
   C11 ≈ C10; if anything the axes mildly antagonize at the edge.
4. **Cost.** The only axis that pays (depth) is also the expensive one (~16×); the
   cheap axis (data) is worthless. So the trade is clear: **spend compute on
   depth, not data.**

### Implications

- **Train deeper, not wider-in-data.** To improve the standalone model, extend the
  size range. We can likely lower the data budget below 4k with no loss (worth
  pinning the floor).
...
- **Does not change the static-composition verdict** (Phase 1). Composition leaves are
  n≤~12, already saturated, so depth can't help there. Consistent with the k-sweep
